<template><!--
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-opacity-75 backdrop-blur-sm" @click.self="emit('close')">
        <div ref="modalContentRef" class="relative flex flex-col w-11/12 max-w-5xl max-h-[90vh] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl m-4 transition-all duration-300">
            <header class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
                ...
                <div class="flex items-center gap-2">
                    <button v-if="data && (data.type === 'txt' || data.type === 'md')" @click="toggleToolbar" class="p-2 text-gray-500 rounded-full hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-700" :title="isToolbarVisible ? 'Ocultar ferramentas' : 'Mostrar ferramentas'">
                        <ChevronUp v-if="isToolbarVisible" class="w-6 h-6" />
                        <ChevronDown v-else class="w-6 h-6" />
                    </button>

                    <button @click="toggleFullScreen" class="p-2 text-gray-500 rounded-full hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-700" :title="isFullScreen ? 'Sair da Tela Cheia' : 'Entrar em Tela Cheia'">
            ...
            <Transition name="slide-fade">
                <section v-if="isToolbarVisible && data && (data.type === 'txt' || data.type === 'md')" class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4 p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 shrink-0">
...
<script setup lang="ts">
    ...
    //import { useLivroVisualizador } from '@/features/biblioteca/livros/composables/useLivroVisualizador';
    ...
    const toggleFullScreen = () => {
        if (!document.fullscreenElement) {
            modalContentRef.value?.requestFullscreen();
        } else {
