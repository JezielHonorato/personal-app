...
<script lang="ts" setup>
    ...
    const { error: errorDeletarLivro, remove } = deleteLivro;

    const editarLivro = (id: number) => {
        router.push(`/livros/editar/${id}`);
    };
    ...
    const apagarLivro = async (id: number) => {
        ...
        remove(id);
        if (errorDeletarLivro.value) console.error('Falha ao apagar o livro:', errorDeletarLivro.value);
        emit('livroApagado', id);
    };
</script>
